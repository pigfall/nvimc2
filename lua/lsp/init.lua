...
-- Auto-install coc-tsserver if missing
...
-- Safely call coc#config('coc.enabled', 1)
   ...
   if ok then
     if vim.fn.exists('*coc#refresh') == 1 then
       ...
       keyset("n", "'si", ":<C-u>CocList -I symbols<cr>", {silent=true,nowait=true})
       vim.api.nvim_create_user_command("CocListSymbols", function(opts)
         local input = opts.args or ""
         input = input:gsub("\\", "\\\\"):gsub('"', '\\"')
         vim.cmd(string.format('CocList -I --input="%s" symbols', input))
       end, { nargs = 1, desc = "CocList symbols with input" })
       keyset("n", "'ss", ":<C-u>ListSymbols", {silent=true,nowait=true})
